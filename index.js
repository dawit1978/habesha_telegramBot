...
/add_ad <ad_type> <ad_link> - Add an advertisement
...
bot.start(async (ctx) => {
    try {
        ...
        // Display advertisements with buttons
        const [ads] = await connection.query('SELECT * FROM advertisements');
        if (ads.length > 0) {
            const buttons = ads.map(ad => Markup.button.url(ad.ad_type, ad.ad_link));
            ctx.reply('Check out our sponsors:', Markup.inlineKeyboard(buttons, { columns: 2 }));
        }
...
bot.command('add_ad', isAdmin, async (ctx) => {
    const [_, adType, adLink] = ctx.message.text.split(' ');
    try {
        const connection = await db;
